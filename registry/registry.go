...
// Resolves provider and format from their respective global registries.
func (r *Registry) Get(name string) (agent.Agent, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to create provider for agent %q: %w", name, err)
	}

	f, err := format.Create(cfg.Format)
	if err != nil {
		return nil, fmt.Errorf("failed to create format for agent %q: %w", name, err)
	}
...
func (r *Registry) Register(name string, cfg config.AgentConfig) error {
	if name == "" {
		return agent.ErrEmptyAgentName
	}
	...
	if _, exists := r.configs[name]; exists {
		return fmt.Errorf("%w: %s", agent.ErrAgentExists, name)
	}
...
func (r *Registry) Replace(name string, cfg config.AgentConfig) error {
	...
	if _, exists := r.configs[name]; !exists {
		return fmt.Errorf("%w: %s", agent.ErrAgentNotFound, name)
	}
